...
const FactureModal: React.FC<FactureModalProps> = ({
  ...
  const { addFacture, updateFacture, generateFactureNumber } = useFactureStore();
  ...
  useEffect(() => {
    if (facture && mode === 'edit') {
      setFormData({
        title: facture.title,
        clientInfo: facture.clientInfo,
        dateCreation: facture.dateCreation.split('T')[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const factureData = {
        ...formData,
        number: facture?.number || generateFactureNumber(),
        status: 'brouillon' as Facture['status'],
        ...totals,
        amountPaid: facture?.amountPaid || 0,
        remainingAmount: facture?.remainingAmount || totals.totalTTC,
        payments: facture?.payments || [],
        createdAt: facture?.createdAt || new Date().toISOString(),
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-6xl sm:w-full max-h-screen overflow-y-auto">
          <form onSubmit={handleSubmit}>
            ...
            <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
              ...
              >
                {isSubmitting ? '💾 Enregistrement...' : (mode === 'create' ? '➕ Créer la facture' : '✏️ Modifier la facture')}
              </button>
              <button
                type="button"
                onClick={onClose}
